...
refsForm.addEventListener('submit', searchQuery);

async function searchQuery(e) {
  e.preventDefault();
  pixabayAPI.query = refsInput.value.trim()
  ...
  refsGallery.innerHTML = ''
    ...
    try {
        ...
        if (response.totalHits) {
            ...
            const { height: cardHeight } = refsGallery.firstElementChild.getBoundingClientRect();
                window.scrollBy({
                  top: cardHeight * -100,
                  behavior: 'smooth',
                })
